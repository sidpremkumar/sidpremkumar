...
def parse(html):
    tips = {}
    for fid, txt in re.findall(
        r'for="(contribution-day-component-\d+-\d+)"[^>]*>([^<]*)</tool-tip>', html
    ):
        t = txt.strip().lower()
        m = re.match(r"([\d,]+)", t)
        tips[fid] = (
            0 if t.startswith("no") else (int(m.group(1).replace(",", "")) if m else 0)
        )
    cells = {}
    for tag in re.findall(r'<td[^>]*class="ContributionCalendar-day"[^>]*>', html):
        did = re.search(r'id="(contribution-day-component-(\d+)-(\d+))"', tag)
        dd = re.search(r'data-date="([0-9-]+)"', tag)
        if did and dd:
            cells[did.group(1)] = (int(did.group(2)), int(did.group(3)), dd.group(1))
    return tips, cells
...
def get_data(user):
    ...
    for cid, (r, c, d) in cells.items():
        ...
        grid[r][c] = v
        ty += v
        if c not in coldate or d < coldate[c]:
            coldate[c] = d
    mx = max(max(r) for r in grid) or 1
    months = []
    seen = set()
    for c in sorted(coldate):
        dt = datetime.date.fromisoformat(coldate[c])
        k = (dt.year, dt.month)
        if k not in seen:
            seen.add(k)
            months.append((c, dt.strftime("%b").upper()))
    life = 0
...
def pts(p):
    return " ".join(f"{x:.0f},{y:.0f}" for x, y in p)
...
def frame_header(W, lt, ty, bd):
    return (
        f'<rect x="10" y="10" width="{W-20}" height="30" rx="9" fill="#06150c"/>'
        ...
        f'<text x="{W//2}" y="29" text-anchor="middle" fill="#3fae6f" font-size="13" letter-spacing="1">sid@nyc: ~/contributions</text>'
...
def svg_shell(W, H, inner, updated):
    return (
        f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}" font-family="\'Courier New\',Courier,monospace">'
...
def anim_once(flat, full, delay):
    return (
        f'<animate attributeName="points" begin="{delay:.2f}s" dur="{GROW}s" '
...
def build(grid, ncols, mx, ty, life, months, animate=True):
    ...
    H = int(maxy - miny + pad * 2 + 86 + LABELBAND)
    ...
    for d in tw:
        ...
        if v == 0:
            body.append(
                f'<polygon points="{pts(faces(cx,yb,HMIN)[2])}" fill="{shade(rgb,1.0)}"/>'
            )
            continue
        if not animate:
            body.append(
                f'<polygon points="{pts(Lf)}" fill="{shade(rgb,0.42)}"/><polygon points="{pts(Rf)}" fill="{shade(rgb,0.66)}"/><polygon points="{pts(Tf)}" fill="{shade(rgb,1.0)}"/>'
            )
            continue
        L0, R0, T0 = faces(cx, yb, 0.0)
        dl = d["delay"]
        for ff, fl, k in ((Lf, L0, 0.42), (Rf, R0, 0.66), (Tf, T0, 1.0)):
            body.append(
                f'<polygon points="{pts(fl)}" fill="{shade(rgb,k)}">{anim_once(pts(fl),pts(ff),dl)}</polygon>'
            )
    ...
    y0 = oy + (0 + R) * HD + HD
    x1 = ox + ((ncols - 1) - R) * HW
    y1 = oy + ((ncols - 1) + R) * HD + HD
    labels.append(
        f'<line x1="{x0:.0f}" y1="{y0:.0f}" x2="{x1:.0f}" y2="{y1:.0f}" stroke="#10532e" stroke-width="1"/>'
    )
    for c, lbl in months:
        bx = ox + (c - R) * HW
        by = oy + (c + R) * HD + HD
        labels.append(
            f'<line x1="{bx:.0f}" y1="{by:.0f}" x2="{bx:.0f}" y2="{by+7:.0f}" stroke="#1c7a44" stroke-width="1.4"/>'
            f'<text x="{bx:.0f}" y="{by+19:.0f}" text-anchor="middle" fill="#43b873" font-size="11" letter-spacing="1">{lbl}</text>'
        )
    inner = (
        frame_header(W, f"{life:,}", f"{ty:,}", str(mx))
